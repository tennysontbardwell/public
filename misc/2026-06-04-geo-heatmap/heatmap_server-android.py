...
Usage:
    ...
    python heatmap_server.py data.json          # then open http://localhost:8000
...
# ============================================================ parsing
def _geo(s):
    # Accept: {"latLng": "43.47°, -80.53°"}, "geo:lat,lon", or "43.47°, -80.53°"
    if isinstance(s, dict):
        s = s.get("latLng") or s.get("placeLocation") or s.get("point") or ""
    s = str(s).replace("geo:", "").replace("\u00b0", "")   # strip prefix + degree sign
    m = re.search(r"(-?\d+(?:\.\d+)?)\s*,\s*(-?\d+(?:\.\d+)?)", s)
    if not m:
        raise ValueError(f"unparseable geo: {s!r}")
        ...
        try:
            if "visit" in e:
                ...
                for p in e["timelinePath"]:
                    g = _geo(p["point"])
                    if "time" in p:
                        secs = (datetime.fromisoformat(p["time"].replace("Z", "+00:00"))
                                - start).total_seconds()
                    else:
                        secs = float(p["durationMinutesOffsetFromStartTime"]) * 60
                    tp.append((g, secs))
